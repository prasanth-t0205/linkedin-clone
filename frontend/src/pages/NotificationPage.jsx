...
const NotificationPage = () => {
  ...
  const { data: notifications, isLoading } = useQuery({
    ...
    queryFn: () => axiosInstance.get("/notification"),
  });

  const { mutate: markNotificationAsRead } = useMutation({
    mutationFn: (id) => axiosInstance.put(`/notification/${id}/read`),
    onSuccess: () => {
  ...
  const renderNotificationIcon = (type) => {
    switch (type) {
      ...
      case "comment":
        return <MessagesSquare className="text-green-500" />;
                    ...
                    <div className="flex gap-2">
                      {!notification.read && (
                        <button
                          ...
                          className="text-blue-600 bg-blue-100 rounded hover:bg-blue-200 transition-colors"
